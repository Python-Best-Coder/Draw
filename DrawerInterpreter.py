...
# 2. Bypass dumpkeys execution inside desktop terminal
...
def draw_kitty(args):
    total_cells = len(Drawer.values)
    
    if cl_ctx and prg and all(isinstance(x, int) for x in Drawer.values):
        vals_np = np.array(Drawer.values, dtype=np.int32)
        ...
        for idx, cell in enumerate(Drawer.values):
...
# Fast variable lookup dict alongside traditional list representation
var_map = {}
watch = {}
variables = [
    {
...
def parse_statement(state):
    ...
    # Match string literals first before operator splitting
    STRING = RE_STRING.match(state)
    if STRING:
        return (STRING.group(1), "str")

    # Fast check for simple integer literals
...
def execute_line(line, i):
    ...
    if Declare:
        ...
        if parsed_val is not None:
            ...
            if var_name in watch and oldval:
